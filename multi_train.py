...
def train(it, train_type):
    ...
    if train_type == 'unpair':
        for _ in range(config.train.batch_acc):
            ...
            # clip_norm_value = clip_grad_norm_(model.parameters(), config.train.clip_norm)
            clip_norm_value = 0.0
            optimizer.step()
        ...
        for it_acc in range(config.train.batch_acc):
            optimizer.zero_grad()
            ...
            H_L_sum_acc_loss += H_L_acc_loss
            sum_roc_auc += roc_auc
            # clip_sum_value += clip_norm_value
    ...
    mean_clip_norm_value = clip_sum_value / config.train.batch_acc

    if rank == 0:
        logger.info('Training iter {}, Loss is: {:.6f} | H_L_loss: {:.6f} | H_L_nll: {:.6f} '
                    '| H_L_cdr_loss: {:.6f} | H_L_acc: {:.6f} | ROC_AUC: {:.6f} | Clip_norm: {:.6f}'.
                    format(it, mean_loss, mean_H_L_loss, mean_H_L_nll,
                           mean_H_L_cdr_loss, mean_H_L_acc_loss, mean_roc_auc, mean_clip_norm_value))
        ...
        writer.add_scalar('train/clip_norm', mean_clip_norm_value, it)
        writer.flush()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--unpair_data_path', type=str,
                        # default='/data/home/waitma/antibody_proj/antidiff/data/oas_unpair_data/'
                        default='/apdcephfs/share_1364275/waitma/anti_proj/data/oas_unpair_human_data/'
                        )
    ...
    parser.add_argument('--config_path', type=str,
                        ...
                        default='/apdcephfs/share_1364275/waitma/anti_proj/v002/configs/training.yml'
                        )
    parser.add_argument('--log_path', type=str,
                        # default='/data/home/waitma/antibody_proj/antidiff/log'
                        default='/apdcephfs/share_1364275/waitma/anti_proj/log_v2'
                        )
    parser.add_argument('--version', type=str,
                        default='v01_not_pretrain')
    parser.add_argument('--resume', type=bool,
                        default=False)
    parser.add_argument('--checkpoint', type=str,
                        default='/apdcephfs/share_1364275/waitma/anti_proj/log/'
                                'v12_mul_2023_12_07__17_55_18/checkpoints/390.pt')
    parser.add_argument('--local_rank', type=int,
                        default=0)

    args = parser.parse_args()

    # Mult training parameters.
    ...
    if not args.resume:
        ...
        # Need to reduce the batchsize, otherwise OOM.
        # Just for pretrain model.
    ...
    if rank == 0:
        ...
        shutil.copyfile('./multi_train.py', os.path.join(log_dir, 'multi_train.py'))
    ...
    if args.data_name == 'pair':
        ...
        h_train_sampler = torch.utils.data.distributed.DistributedSampler(h_train_dataset, shuffle=True)
        h_val_sampler = torch.utils.data.distributed.DistributedSampler(h_val_dataset, shuffle=True)
        l_train_sampler = torch.utils.data.distributed.DistributedSampler(l_train_dataset, shuffle=False)
        l_val_sampler = torch.utils.data.distributed.DistributedSampler(l_val_dataset, shuffle=False)
    ...
    model = model_selected(config).to(local_rank)
    ...
    if args.resume:
        ...
        if rank == 0:
            iteration_start = ckpt['iteration']
            logger.info('The re iteration start from {}'.format(iteration_start))
